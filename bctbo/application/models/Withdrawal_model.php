...
class Withdrawal_model extends CI_Model {
	...
	{	
		...
		$DBdata = array(
			...
			'amount' => $arr['amount'],
			'remark' => !empty($remark) ? $remark : $arr['remark'],
	...
	// Get withdrawal data by player id
	public function getWithDrawalDataByPlayerId($player_id)

	{	
		 // get today
		 $today = strtotime(date('Y-m-d')); 							// get today convert to timestamp

		$this->db->select('player_id, COUNT(*) as status_1_count');
		$this->db->where('status', 1);
		$this->db->where('player_id', $player_id);
		
		// set where today
		$this->db->where('created_date >=', $today); 					// compare greater than or equal today
		$this->db->where('created_date <', $today + 86400); 			// compare less than today + 1 day

		$this->db->group_by('player_id');

		$query = $this->db->get($this->table_withdrawals);
		...
		if ($query->num_rows() > 0) 
		{
			...
			{
				$result[$row->player_id] = $row->status_1_count;
			}
		}
	...
	public function week_total_only_agent_withdrawal(){		$result['total'] = 0;		$result_row = array();				$dbprefix = $this->db->dbprefix;		$start_date = strtotime('monday this week');		$end_date = strtotime('sunday this week') + (24 * 60 * 60 - 1);		$where = "";		$where .= ' AND a.updated_date >= ' . $start_date;		$where .= ' AND a.updated_date <= ' . $end_date;		$where .= ' AND a.status = '. STATUS_APPROVE;		$result = NULL;		$query_string = "(SELECT SUM(a.amount) AS total FROM {$dbprefix}withdrawals a, {$dbprefix}players b WHERE (a.player_id = b.player_id) AND b.upline_ids LIKE '%," . $this->session->userdata('root_user_id') . ",%' ESCAPE '!' $where)";		/*		$dbprefix = $this->db->dbprefix;		$where .= ' AND a.report_date >= ' . strtotime(date('Y-m-d 00:00:00'));		$where .= ' AND a.report_date <= ' . strtotime(date('Y-m-d 23:59:59'));		$where .= ' AND a.transfer_type IN (' . TRANSFER_POINT_OUT . ', ' . TRANSFER_ADJUST_OUT . ', ' . TRANSFER_WITHDRAWAL . ')';		$query_string = "SELECT SUM(a.withdrawal_amount) AS total FROM {$dbprefix}cash_transfer_report a, {$dbprefix}players b WHERE (a.username = b.username) AND b.upline_ids LIKE '%," . $this->session->userdata('root_user_id') . ",%' ESCAPE '!' $where";		*/		$query = $this->db->query($query_string);		if($query->num_rows() > 0)		{			$result_row = $query->row_array();  			if(!empty($result_row['total'])){			    $result['total'] += $result_row['total'];   			}		}		$query->free_result();		$dbprefix = $this->db->dbprefix;		$where = "";		$where .= ' AND a.report_date >= ' . strtotime(date('Y-m-d 00:00:00'));		$where .= ' AND a.report_date <= ' . strtotime(date('Y-m-d 23:59:59'));		$where .= ' AND a.transfer_type IN (' . TRANSFER_POINT_OUT . ')';		$query_string = "SELECT SUM(a.withdrawal_amount) AS total FROM {$dbprefix}cash_transfer_report a, {$dbprefix}players b WHERE (a.username = b.username) AND b.upline_ids LIKE '%," . $this->session->userdata('root_user_id') . ",%' ESCAPE '!' $where";		$query = $this->db->query($query_string);		if($query->num_rows() > 0)		{			$result_row = $query->row_array();  			if(!empty($result_row['total'])){			    $result['total'] += $result_row['total'];   			} 		}				$query->free_result();		return $result;	}	public function month_total_only_agent_withdrawal(){		$result['total'] = 0;		$result_row = array();				$dbprefix = $this->db->dbprefix;		$start_date = mktime(0, 0, 0, date('m'), 1, date('Y'));		$end_date = mktime(23, 59, 59, date('m'), date('t'), date('Y'));		$where = "";		$where .= ' AND a.updated_date >= ' . $start_date;		$where .= ' AND a.updated_date <= ' . $end_date;		$where .= ' AND a.status = '. STATUS_APPROVE;		$result = NULL;		$query_string = "(SELECT SUM(a.amount) AS total FROM {$dbprefix}withdrawals a, {$dbprefix}players b WHERE (a.player_id = b.player_id) AND b.upline_ids LIKE '%," . $this->session->userdata('root_user_id') . ",%' ESCAPE '!' $where)";		/*		$dbprefix = $this->db->dbprefix;		$where .= ' AND a.report_date >= ' . strtotime(date('Y-m-d 00:00:00'));		$where .= ' AND a.report_date <= ' . strtotime(date('Y-m-d 23:59:59'));		$where .= ' AND a.transfer_type IN (' . TRANSFER_POINT_OUT . ', ' . TRANSFER_ADJUST_OUT . ', ' . TRANSFER_WITHDRAWAL . ')';		$query_string = "SELECT SUM(a.withdrawal_amount) AS total FROM {$dbprefix}cash_transfer_report a, {$dbprefix}players b WHERE (a.username = b.username) AND b.upline_ids LIKE '%," . $this->session->userdata('root_user_id') . ",%' ESCAPE '!' $where";		*/		$query = $this->db->query($query_string);		if($query->num_rows() > 0)		{			$result_row = $query->row_array();  			if(!empty($result_row['total'])){			    $result['total'] += $result_row['total'];   			}		}		$query->free_result();		$dbprefix = $this->db->dbprefix;		$where = "";		$where .= ' AND a.report_date >= ' . strtotime(date('Y-m-d 00:00:00'));		$where .= ' AND a.report_date <= ' . strtotime(date('Y-m-d 23:59:59'));		$where .= ' AND a.transfer_type IN (' . TRANSFER_POINT_OUT . ')';		$query_string = "SELECT SUM(a.withdrawal_amount) AS total FROM {$dbprefix}cash_transfer_report a, {$dbprefix}players b WHERE (a.username = b.username) AND b.upline_ids LIKE '%," . $this->session->userdata('root_user_id') . ",%' ESCAPE '!' $where";		$query = $this->db->query($query_string);		if($query->num_rows() > 0)		{			$result_row = $query->row_array();  			if(!empty($result_row['total'])){			    $result['total'] += $result_row['total'];   			} 		}				$query->free_result();		return $result;	}	
