...
class Player_model extends CI_Model {
	...
	// Update player point
	public function updatePlayerPoint($player_id, $point) {
		$this->db->set('points', $point);
	...
	{	
		...
		if(permission_validation(PERMISSION_PLAYER_NICKNAME) == TRUE){
			$DBdata['full_name'] = $this->input->post('fullname', TRUE);
		}
		if(permission_validation(PERMISSION_PLAYER_LINE_ID) == TRUE){
			$DBdata['line_id'] = $this->input->post('line_id', TRUE);
		}
	...
	public function updateRemark($arr = NULL)
	{	
	...
	public function updatePasswordBank($arr = NULL)
	{	
		$new_password = $this->input->post('password', TRUE);
		// $new_password = password_hash($new_password, PASSWORD_DEFAULT);
		$DBdata = array(
			'withdrawal_password' => $new_password,
			'updated_by' => $this->session->userdata('username'),
			'updated_date' => time()
		);
		$this->db->where('player_id', $arr['player_id']);
	...
	{	
		...
		// Sau khi cập nhật, truy về giá trị mới của points
		$updatedPoints = $this->db->query("SELECT points FROM {$table} WHERE player_id = ?", $DBdata['player_id'])->row_array();
    
		return $updatedPoints['points'];
		// return $DBdata;
	}

	public function update_level_id($player_id, $level_id)
	{	
		$DBdata = array(
			'level_id' => $level_id,
			'updated_by' => $this->session->userdata('username'),
	...
	/**
	 * Get number pending agent in bctp_users_agents
	 */
	public function getAgentPending(){
		
		$dbprefix = $this->db->dbprefix;
		$queryString = "SELECT count(1) as total_agent_pending FROM {$dbprefix}users_agents";
		$resultQuery = $this->db->query($queryString);
		$agentPending = NULL;
		if($resultQuery->num_rows() > 0)
		{
			$agentPending =  $resultQuery->row_array();  
		}
	...
	// 	total_bet: The total number of bets made by the player at the present time.
	// 	total_bet_amount: The total bet amount of the player at the present time.
	// 	total_win_loss: The total amount of winnings and losses of the player at the present time.
	// 	total_rolling_amount: Total valid bet amount of the player at the present time.
	// 	total_profit: total_rolling_amount - total_win_loss
	// 	total_profit_percentage: total_profit / total_rolling_amount * 100
	...
	public function player_week_win_loss(){
		$result = NULL;
		$start_date = strtotime('monday this week');
		$end_date = strtotime('sunday this week') + (24 * 60 * 60 - 1);
		$dbprefix = $this->db->dbprefix;
		$where = "";
		$where .= ' AND a.status = ' . STATUS_COMPLETE;
		$where .= ' AND a.payout_time >= ' . $start_date;
	...
	public function player_month_win_loss(){
		$result = NULL;
	...
	/**
	 * Get total deposit by player id
	 */
	public function getTotalDepositByPlayer($player_id){

	    $totalDeposit = 0;    
		$dbprefix = $this->db->dbprefix;

		$sql = "SELECT SUM(amount_apply) AS total FROM {$dbprefix}deposits WHERE player_id = " . $player_id . " AND status = " . STATUS_APPROVE;
		$query = $this->db->query($sql);

		$depositInfo = array();
		if($query->num_rows() > 0)
		{
			$depositInfo = $query->row_array();  

			if(!empty($depositInfo['total'])){

			    $totalDeposit += $depositInfo['total'];   
		...
		$sql = "SELECT SUM(a.deposit_amount) AS total FROM {$dbprefix}cash_transfer_report a, {$dbprefix}players b 
				WHERE (a.username = b.username) AND a.transfer_type IN (" . TRANSFER_POINT_IN . ") AND b.player_id = " . $player_id;

		$query = $this->db->query($sql);
		if($query->num_rows() > 0)
		{
			$cashTransfer = $query->row_array();  

			if(!empty($cashTransfer['total'])){

			    $totalDeposit += $cashTransfer['total'];   
	...
	/**
	 * Get total withdrawal by player id
	 */
	public function getTotalWithdrawalByPlayer($player_id){

	    $totalWithdrawal = 0;    
		$dbprefix = $this->db->dbprefix;

		$sql = "SELECT SUM(withdrawal_amount) AS total FROM {$dbprefix}cash_transfer_report a, {$dbprefix}players b 
				WHERE (a.username = b.username) AND a.transfer_type IN (" . TRANSFER_POINT_OUT . "," . TRANSFER_ADJUST_OUT . "," . TRANSFER_WITHDRAWAL . ") AND b.player_id = " . $player_id;
		...
		if($query->num_rows() > 0)
		{
			...
			if(!empty($cashTransfer['total'])){
			    ...
			    $totalWithdrawal += $cashTransfer['total'];   
	...
	/**
	 * Get total bonus by player id
	 */
	public function getTotalBonusByPlayer($player_id){

	    $totalBonus = 0;    
		$dbprefix = $this->db->dbprefix;

		$sql = "SELECT SUM(a.deposit_amount) AS total FROM {$dbprefix}cash_transfer_report a, {$dbprefix}players b 
				WHERE (a.username = b.username) AND a.transfer_type IN (" . TRANSFER_BONUS . ") AND b.player_id = " . $player_id;
	...
	/**
	 * Get total transfer in by player id
	 */
	public function getTotalTransferInByPlayer($player_id){
		...
		if($query->num_rows() > 0)
		{
			...
			if(!empty($cashTransfer['total'])){
			    ...
			    $totalTransferIn += $cashTransfer['total'];   
	...
	/**
	 * Get total transfer out by player id
	 */
	public function getTotalTransferOutByPlayer($player_id){

	    $totalTransferOut = 0;    
		$dbprefix = $this->db->dbprefix;

		$sql = "SELECT SUM(a.withdrawal_amount) AS total FROM {$dbprefix}cash_transfer_report a, {$dbprefix}players b 
				WHERE (a.username = b.username) AND a.transfer_type IN (" . TRANSFER_POINT_OUT . ") AND b.player_id = " . $player_id;
		...
		if($query->num_rows() > 0)
		{
			...
			if(!empty($cashTransfer['total'])){
			    ...
			    $totalTransferOut += $cashTransfer['total'];   
	...
	/**
	 * Get total adjustment in by player id
	 */
	public function getTotalAdjustInByPlayer($player_id){

	    $totalAdjustIn = 0;    
		$dbprefix = $this->db->dbprefix;

		$sql = "SELECT SUM(a.deposit_amount) AS total FROM {$dbprefix}cash_transfer_report a, {$dbprefix}players b 
				WHERE (a.username = b.username) AND a.transfer_type IN (" . TRANSFER_ADJUST_IN . ") AND b.player_id = " . $player_id;
		...
		if($query->num_rows() > 0)
		{
			...
			if(!empty($cashTransfer['total'])){
			    ...
			    $totalAdjustIn += $cashTransfer['total'];   

			}
		}
		$query->free_result();
		return $totalAdjustIn;
	}

	/**
	 * Get total adjustment out by player id
	 */
	public function getTotalAdjustOutByPlayer($player_id){

	    $totalAdjustOut = 0;    
		$dbprefix = $this->db->dbprefix;

		$sql = "SELECT SUM(a.withdrawal_amount) AS total FROM {$dbprefix}cash_transfer_report a, {$dbprefix}players b 
				WHERE (a.username = b.username) AND a.transfer_type IN (" . TRANSFER_ADJUST_OUT . ") AND b.player_id = " . $player_id;
		...
		if($query->num_rows() > 0)
		{
			...
			if(!empty($cashTransfer['total'])){
			    ...
			    $totalAdjustOut += $cashTransfer['total'];   

			}
		}
		$query->free_result();
		return $totalAdjustOut;
	}

	/**
	 * Get total winloss out by player id
	 */
	public function getTotalWinLossByPlayer($player_id){

		$totalWinLoss = 0;  
		$dbprefix = $this->db->dbprefix;			
		$sql = "SELECT COALESCE(SUM(a.win_loss),0) AS total FROM {$dbprefix}transaction_report a, {$dbprefix}players b 
				WHERE (a.player_id = b.player_id) AND a.status = " . STATUS_COMPLETE . " AND a.player_id = " . $player_id;
		$query = $this->db->query($sql);
		if($query->num_rows() > 0)
		{
			$transactionReport =  $query->row_array();  
			if(!empty($transactionReport['total'])){

			    $totalWinLoss += $transactionReport['total'];   
	...
	/**
	 * Update upline for player
	 */
	public function updateUpline($player_id, $upline){

		$table_users = $this->db->dbprefix . "users";
		$sql = "SELECT user_id, upline_ids FROM " . $table_users . " WHERE username = '" . $upline . "' LIMIT 1";
		$query = $this->db->query($sql);
		$uplineIds = ",1,";
		if($query->num_rows() > 0)
		{
			$uplineIds_rs = $query->row_array();  

			if(!empty($uplineIds_rs['upline_ids'])){

				$uplineIds = $uplineIds_rs['upline_ids'] . $uplineIds_rs['user_id'] .",";

			}
		}
		$query->free_result();

		$table_players = $this->db->dbprefix . $this->table_players;
		$sql = "UPDATE {$table_players} SET upline = '" . $upline . "', upline_ids = '" . $uplineIds . "' WHERE player_id = " . $player_id . " LIMIT 1";
		$this->db->query($sql);
	}

	public function getPlayerByBirth($promotionId, $todayDay, $todayMonth)
	{
		$firstDayOfYear = strtotime(date('Y-m-d H:i:s', strtotime('1/1 00:00:00')));

		$lastDayOfYear = strtotime(date('Y-m-d H:i:s', strtotime('12/31 23:59:59')));

		$result = NULL;

		$query = $this->db

			->select('players.player_id, players.dob, from_unixtime(dob) as birth, players.level_id, players.points')
			
			->join('player_promotion',
			
				'players.player_id = player_promotion.player_id

				and player_promotion.promotion_id = "' . $promotionId . '"

				and player_promotion.created_date > "' . $firstDayOfYear . '"

				and player_promotion.created_date < "' . $lastDayOfYear . '"', 'left')

			->where('player_promotion.player_id is null')
			
			->having("DATE_FORMAT(DATE(birth), '%d%m') = " . $todayDay . $todayMonth)
	...
	public function addPointsByPromotion($arr = null) {
		
		$currentPoint = $this->getCurrentPoint($arr['player_id']);
		$newTotalPoint = $currentPoint + $arr['amount'];
	
		$data = array(
			'points' => $newTotalPoint,
		);
	
		$this->db->where('player_id', $arr['player_id']);
		$this->db->update($this->table_players, $data);
	
		return $data;
	}

	public function getCurrentPoint($playerId) {
