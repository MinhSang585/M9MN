...
class Level extends MY_Controller {
	...
	public function index(){
		if(permission_validation(PERMISSION_LEVEL_VIEW) == TRUE)
		{
			...
			// $data['calculate_type'] = $this->level_model->getCalculateType();
	...
	public function listing(){
		if(permission_validation(PERMISSION_LEVEL_VIEW) == TRUE)
		{
			...
			$columns = array(
				...
				2 => 'daily_withdrawal_frequency',

				3 => 'level_number',

				4 => 'level_deposit_amount_from',

				5 => 'level_deposit_amount_to',

				6 => 'level_target_amount_from',

				7 => 'level_target_amount_to',

				8 => 'level_reward_amount',
			...
			if(!empty($posts))
			{
				foreach ($posts as $post)
				{
					...
					$row[] = $post->daily_withdrawal_frequency;
	 ...
	 */
	public function calculate_type_update(){
		...
		if ($this->form_validation->run() == TRUE){
			...
			if(empty($calculate_type)){
				$json['msg']['calculate_type_error'] = $this->lang->line('error_select_calculate_type');
			} else {
				$json['status'] = EXIT_SUCCESS;
				$json['msg'] = $this->lang->line('success_updated');
				$this->level_model->updateCalculateType($calculate_type);
			}
