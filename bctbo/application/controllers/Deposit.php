...
class Deposit extends MY_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('deposit_model', 'level_model', 'player_model','promotion_model','player_promotion_model', 'miscellaneous_model', 'game_model','currencies_model','promotion_apply_model','promotion_approve_model','message_model','tag_model','bank_model'));
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if( ! empty($oldData) && $oldData['status'] == STATUS_PENDING) {
					...
					if($master_flag) {						
						...
						// Update status
						// $this->player_promotion_model->updateStatus($oldData['player_id']);
						...
						if($newData['status'] == STATUS_APPROVE) {
							...
							$points = $this->player_model->update_player_wallet($newData);

							// $points = 120000; 240000

							$getLevels = $this->level_model->getLevelDataNew();
							...
							foreach ($getLevels as $getlevel) {
								...
								$levelTarget = $getlevel['level_target_amount_from'];
							
								if ($points <= $levelTarget) {
									
									$levelIdNew = isset($getLevels[$i-1]['level_id']) ? $getLevels[$i-1]['level_id'] : 1;
									break; 
								}
								$i++;
							}
							...
							$this->player_model->update_level_id($playerId, $levelIdNew);
								...
								if(!empty($PromotionDepositPending)){
									...
									if($promotion_response['status'] == EXIT_SUCCESS){
										...
										if($PromotionDepositPending['reward_on_apply'] == STATUS_ACTIVE){
											...
											$this->player_promotion_model->updateStatusWelcomeBonus($oldData['player_id'], STATUS_ENTITLEMENT, $PromotionDepositPending);
										}
									}
								}
							}else{
