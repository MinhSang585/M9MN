...
class Withdrawal extends MY_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('withdrawal_model', 'level_model', 'player_model', 'player_promotion_model','currencies_model','payment_gateway_model','bank_model','promotion_apply_model','promotion_approve_model','message_model','tag_model', 'promotioncron_model', 'deposit_model'));
	...
	public function listing()
    {
		if(permission_validation(PERMISSION_WITHDRAWAL_VIEW) == TRUE)
		{
			...
			$columns = array( 
				...
				7 => 'a.bank_account_no',
				8 => 'a.bank_account_address',
				9 => 'a.amount',
				10 => 'a.amount_rate',
				11 => 'a.withdrawal_fee_value',
				12 => 'a.withdrawal_fee_amount',
				13 => 'a.status',
				14 => 'a.withdrawal_ip',
				15 => 'a.remark',
				16 => 'a.updated_by',
			...
			if(!empty($posts))
			{
				...
				foreach ($posts as $post)
				{
					...
					$row[] = $post->bank_account_address.'&nbsp;<i class="fas fa-copy nav-icon text-purple clipboard" data-clipboard-text="'.$post->bank_account_address.'"></i>&nbsp;';
					$row[] = '<span class="text-' . (($post->amount > 0) ? 'dark' : 'dark') . '">' . number_format($post->amount, 0, '.', ',') . '</span>';
					$row[] = '<span class="text-' . (($post->amount_rate > 0) ? 'dark' : 'dark') . '">' . number_format($post->amount_rate, 0, '.', ',') . '</span>';
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if(!empty($oldData) && ($oldData['status'] == STATUS_PENDING)) {
					...
					if($allowed_update){
						...
						if(!empty($player)){
							// Handle Online Withdrawal (Bank transfer)
							...
							// Handle Offline Withdrawal (Database update)
							else{
								#Offline Withdrawal
								#Database update
								$this->db->trans_start();
								...
								if($newData['status'] == STATUS_CANCEL)
								{
									...
									// Start Handle daily withdrawal frequency
									
										if($newData['amount'] >= 10000){																		    // If user withdraw more than 10,000, deduct 5% of the amount
											$discount = $newData['amount'] * 5/100;	
											$playerNewId = $this->player_model->get_player_data($newData['player_id']);
											$pointsDiscount = $playerNewId['points'] - $discount;
											$this->player_model->updatePlayerPoint($newData['player_id'], $pointsDiscount);
										}

										$countWiths = $this->withdrawal_model->getWithDrawalDataByPlayerId($newData['player_id']);					// get all withdrawal data by player_id

										$timestamp_today = time();																					// get today timestamp

										foreach ($countWiths as $playerId => $countWith) {

											$playerNewId = $this->player_model->get_player_data($playerId);											// get player data by player_id (of table_player)

											$username  = $playerNewId['username'];

											if ($playerNewId !== null) {

												$level = $this->level_model->get_level_data_by_id($playerNewId['level_id']);						// get level data by level_id

												if($countWith > $level['daily_withdrawal_frequency'])												// if user withdraw more than daily_withdrawal_frequency													
												{
													$percentBase = 5;
													$percentRatio = $percentBase/100;																// 5% percent

													$percentRate = $newData['amount'] * $percentRatio;												// 5% of amount
													$pointNew = $playerNewId['points'] - $percentRate;

													$this->player_model->updatePlayerPoint($playerNewId['player_id'], $pointNew);					// update player point -> new point
												}

												$dataLogWithdrawals = array(

													'player_id' => $playerNewId['player_id'],
													'username' => $playerNewId['username'],
													'date_withdrawal' => $timestamp_today,
													'percent_ratio' => $percentBase,
													'percent_rate' => $percentRate,
													'old_point' => $playerNewId['points'],
													'new_point' => $pointNew,
													'count_withdrawal' => $countWith,
													'daily_withdrawal_frequency' => $level['daily_withdrawal_frequency'],
													'created_by' => $this->session->userdata('username'),
												);

												$this->db->insert('log_withdrawals', $dataLogWithdrawals);											// insert log_withdrawals
											} 
										}

									// End Handle daily withdrawal frequency
								}
	...
	public function withdrawal_offline_add($id = NULL)
	{
		if(permission_validation(PERMISSION_WITHDRAWAL_ADD) == TRUE)
		{
			...
			if( ! empty($data))
			{
				...
				if( ! empty($response))
				{
					$data['bank_list'] = $this->bank_model->getBankListById($playerId);
	...
	public function withdrawal_offline_submit()
	{
		if(permission_validation(PERMISSION_WITHDRAWAL_ADD) == TRUE)
		{
			...
			$bankInfo = trim($this->input->post('bank_id', TRUE));
			$bank_id = explode('!@#$', $bankInfo)[0];
			$accountNo = explode('!@#$', $bankInfo)[1];
			...
			//$bankData = $this->bank_model->getBankData($bank_id);
			$bankData = $this->bank_model->getBanktByAccountNo($player_id, $accountNo);

			$bankName = $this->bank_model->getBankName($bank_id);
				...
				if( ! empty($response))
				{
					...
					$config = array(
						...
						array(
							...
							'rules' => 'trim|required',
							'errors' => array(
								'greater_than' => $this->lang->line('error_select_verify_bank')
							)
						)
					);		
					...
					if ($this->form_validation->run() == TRUE)
					{
						...
						$data = array(
							...
							'bank_id' => $currenciesData['bank_id'],
							...
							'remark' => 'Offline withdrawal executed by admin ' . $this->session->userdata('username')
						);
						//Database update
						$this->db->trans_start();
