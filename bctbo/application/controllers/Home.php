...
class Home extends MY_Controller {
	...
	public function month_deposit() {
		$deposit 	= $this->deposit_model->month_total_only_agent_deposit();
		$value		= ((isset($deposit['total']) && $deposit['total'] > 0)) ? val_decimal($deposit['total'],2) : val_decimal(0,2);
		$total_deposit_formatted = number_format($value, 2, '.', ',');
		$json 		= array('status' => EXIT_SUCCESS,'msg' => $this->lang->line('label_successful'),'result' => $total_deposit_formatted);
		json_output($json);
		free_array($json);	
		exit();	
	}
	public function week_deposit() {
		$deposit 	= $this->deposit_model->week_total_only_agent_deposit();
		$value		= ((isset($deposit['total']) && $deposit['total'] > 0)) ? val_decimal($deposit['total'],2) : val_decimal(0,2);
		$total_deposit_formatted = number_format($value, 2, '.', ',');
		$json 		= array('status' => EXIT_SUCCESS,'msg' => $this->lang->line('label_successful'),'result' => $total_deposit_formatted);
		json_output($json);
	...
	public function today_promotion() {
		...
		$total_promotion_formatted = number_format($value, 2, '.', ',');
		$json 		= array('status' => EXIT_SUCCESS,'msg' => $this->lang->line('label_successful'),'result' => $total_promotion_formatted);
		json_output($json);
		free_array($json);	
		exit();	
	}
	public function month_bonus() {
		$bonus 	= $this->bonus_model->month_total_bonus();
		$value		= ((isset($bonus['total']) && $bonus['total'] > 0)) ? val_decimal($bonus['total'],2) : val_decimal(0,2);
		$total_bonus_formatted = number_format($value, 2, '.', ',');
		$json 		= array('status' => EXIT_SUCCESS,'msg' => $this->lang->line('label_successful'),'result' => $total_bonus_formatted);
		json_output($json);
		free_array($json);	
		exit();	
	}
	public function week_bonus() {
		$bonus 	= $this->bonus_model->week_total_bonus();
		$value		= ((isset($bonus['total']) && $bonus['total'] > 0)) ? val_decimal($bonus['total'],2) : val_decimal(0,2);
		$total_bonus_formatted = number_format($value, 2, '.', ',');
		$json 		= array('status' => EXIT_SUCCESS,'msg' => $this->lang->line('label_successful'),'result' => $total_bonus_formatted);
		json_output($json);
	...
	public function month_withdraw() {
		$withdrawal = $this->withdrawal_model->month_total_only_agent_withdrawal();
		$value		= ((isset($withdrawal['total']) && $withdrawal['total'] > 0)) ? val_decimal($withdrawal['total'],2) : val_decimal(0,2);
		$total_withdraw_formatted = number_format($value, 2, '.', ',');
		$json 		= array('status' => EXIT_SUCCESS,'msg' => $this->lang->line('label_successful'),'result' => $total_withdraw_formatted);
		json_output($json);
		free_array($json);	
		exit();	
	}
	public function week_withdraw() {
		$withdrawal = $this->withdrawal_model->week_total_only_agent_withdrawal();
		$value		= ((isset($withdrawal['total']) && $withdrawal['total'] > 0)) ? val_decimal($withdrawal['total'],2) : val_decimal(0,2);
		$total_withdraw_formatted = number_format($value, 2, '.', ',');
		$json 		= array('status' => EXIT_SUCCESS,'msg' => $this->lang->line('label_successful'),'result' => $total_withdraw_formatted);
		json_output($json);
	...
	public function month_profit() {
		$total_rolling_amount = $this->player_model->player_month_win_loss()['total_rolling_amount'];
	
		$total_win_loss = $this->player_model->player_month_win_loss()['total_win_loss'];
		...
		$json = array(
		'status' => EXIT_SUCCESS,
		'msg' => $this->lang->line('label_successful'),
		'result' => $total_profit_formatted
		);
		json_output($json);
		free_array($json);
		exit();
	...
	public function week_profit() {
		$total_rolling_amount = $this->player_model->player_week_win_loss()['total_rolling_amount'];
	
		$total_win_loss = $this->player_model->player_week_win_loss()['total_win_loss'];
	...
	public function today_profit() {
		...
		$total_profit_formatted = number_format($value, 2, '.', ',');
		$json 		= array('status' => EXIT_SUCCESS,'msg' => $this->lang->line('label_successful'),'result' => $total_profit_formatted);
		json_output($json);
		free_array($json);	
		exit();	
	}
	public function today_user() {
		...
		$total_user_formatted = number_format($value, 2, '.', ',');
		$json 		= array('status' => EXIT_SUCCESS,'msg' => $this->lang->line('label_successful'),'result' => $total_user_formatted);
		json_output($json);
		free_array($json);
		free_array($player);	
		exit();	
	...
	*/
	public function agentPending(){
		...
		$agentPending = $this->player_model->getAgentPending();
		$totalAgentPending = (isset($agentPending['total_agent_pending'])) ? $agentPending['total_agent_pending'] : 0;
		$json = array('status' => EXIT_SUCCESS,'msg' => $this->lang->line('label_successful'),'totalAgentPending' => $totalAgentPending);
		json_output($json);
		exit();
	}
	...
	public function totalDepositOnlineInterval(){
		...
		$where .= ' AND a.status = ' . STATUS_ON_HOLD_PENDING;
		$query_string = "(SELECT {$select} FROM {$dbprefix}deposits a, {$dbprefix}players b, {$dbprefix}avatar c WHERE (a.player_id = b.player_id) AND (b.avatar = c.avatar_id) AND b.upline_ids LIKE '%," . $this->session->userdata('root_user_id') . ",%' ESCAPE '!' $where)";
		$query_string_2 = " ORDER by a.created_date DESC LIMIT {$start}, {$limit}";
		//log_message('error', print_r('logSound', true));
		//log_message('error', print_r($query_string.$query_string_2, true));
		$query = $this->db->query($query_string.$query_string_2);
	...
	public function set_announcement($type = ANNOUNCEMENT_DEPOSIT){
		// log_message('error', print_r('set_announcement_home', true));
