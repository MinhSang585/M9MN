...
class Marquee extends MY_Controller {
...

	{
		...
		if(permission_validation(PERMISSION_MARQUEE_VIEW) == TRUE)

		{

			$this->save_current_url('marquee');

			

			$data['page_title'] = $this->lang->line('title_marquee');

			$this->load->view('marquee_view', $data);
...

    {
		...
		if(permission_validation(PERMISSION_MARQUEE_ADD) == TRUE)

		{

			$this->load->view('marquee_add');
...

	{
...

		{
...

			{
				...
				$newData = $this->marquee_model->add_marquee();
				...
				if(sizeof($lang)>0){
					...
					foreach($lang as $k => $v){
						...
						$this->marquee_model->add_marquee_lang($newData['announcement_id'],$v);

					}

				}

				$newLangData = $this->marquee_model->get_marquee_lang_data($newData['announcement_id']);
...

				{
					...
					$this->user_model->insert_log(LOG_MARQUEE_ADD, $newData);
...

				{
					...
					$this->account_model->insert_log(LOG_MARQUEE_ADD, $newData);
    ...
    {
		if(permission_validation(PERMISSION_MARQUEE_UPDATE) == TRUE)
		{
			$data = $this->marquee_model->get_marquee_data($id);

			$data['marquee_lang'] = $this->marquee_model->get_marquee_lang_data($id);

			$this->load->view('marquee_update', $data);
		}
		else
		{
			redirect('home');
	...
	public function update()
	{
		if(permission_validation(PERMISSION_MARQUEE_UPDATE) == TRUE)
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				$marquee_id = trim($this->input->post('marquee_id', TRUE));

				$oldData = $this->marquee_model->get_marquee_data($marquee_id);

				$oldLangData = $this->marquee_model->get_marquee_lang_data($marquee_id);
				...
				if( ! empty($oldData))
				{
					...
					$this->marquee_model->delete_marquee_lang($marquee_id);
					...
					if(sizeof($lang)>0){
						...
						foreach($lang as $k => $v){
							...
							$this->marquee_model->add_marquee_lang($marquee_id,$v);

						}
					}

					$newLangData = $this->marquee_model->get_marquee_lang_data($marquee_id);

					$newData = $this->marquee_model->update_marquee($marquee_id);

					$newData['lang'] = json_encode($newLangData);

					if($this->session->userdata('user_group') == USER_GROUP_USER) 
					{
						$this->user_model->insert_log(LOG_MARQUEE_UPDATE, $newData, $oldData);
					}
					else
					{
						$this->account_model->insert_log(LOG_MARQUEE_UPDATE, $newData, $oldData);
					}
	...
	public function delete()
    {
		...
		if(permission_validation(PERMISSION_MARQUEE_DELETE) == TRUE)
		{
			$marquee_id = $this->uri->segment(3);
			...
			if( ! empty($oldData))
			{
				...
				$this->marquee_model->delete_marquee($marquee_id);

				$this->marquee_model->delete_marquee_lang($marquee_id);

				if($this->session->userdata('user_group') == USER_GROUP_USER) 
				{
					$this->user_model->insert_log(LOG_MARQUEE_DELETE, $oldData);
				}
				else
				{
					$this->account_model->insert_log(LOG_MARQUEE_DELETE, $oldData);
				}
