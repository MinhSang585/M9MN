...
var _Main = {
			...
			$("[data-dom=announcement]").each(function () {
				...
				_Util.post(
					...
					function (data) {
						...
						if (setting.add_marquee_tag) announcement += "<marquee>";
	...
	getWebEvent: function () {
		if (["index", "main"].indexOf(_p) >= 0) {
			_Util.post(
				"a/getWebEvent?ct=" + _currenthour + "&" + lang + (typeof loginUser != 'undefined'? '&' + loginUser : ''),
				{
	...
	showWebEvent: function () {
		...
		if (_Main.vars.webEventList.length > 0) {
			if (_Main.vars.webEventList[0].embeddedvideo == 0) {
				...
				str = _Main.vars.webEventList[0].url; //image
			}
					...
					if (_Main.vars.webEventList[0].embeddedvideo == 0) {
						...
						if (_Util.isMobile()) {
							$("<img/>") // Make in memory copy of image to avoid css issues
								.attr("src", _Main.vars.webEventList[0].image)
								.load(function () {
...
var _Ruler = {
	load: function (cb) {
		if (typeof _rules == "undefined") {
			$.getScript(defJsUrl + "js/rules.js")
				.done(function () {
					cb();
				})
				.fail(function (j, s, e) {
...
var _User = {
	...
	getUnreadMessage: function () {
		if ($("[data-dom=mailbox]:not(.dom-registered)").length > 0 && loginFlag) {
			_Util.post("a/getPrivateMessage", { action: "unread" }, function (data) {
				if (data.status == "0") {
		...
		if ($("[data-dom=register-form]:not(.dom-registered)").length > 0) {
			$("[data-dom=register-form]").each(function () {
				...
				_Ruler.load(function () {
					...
					_Validator.vars.validation = _form.validate(
						$.extend(r, {
							errorPlacement: function (err, $e) {
								var name = $e.attr("name");
												...
												if (reg.error.length > 0) {
													...
													if (typeof _ctrl !== "undefined" && _ctrl !== null) {
														if (typeof _ctrl.gotoThankYou == "function") {
															var ct = '';
															$.when(
																$.getScript(defImgPath + "js/visitor.js?v=1.5"),
																$.getScript(defImgPath+"/js/fp.min.js?v=1.5"),
																...
																fpPromise
																	...
																	.then(result => {
																		...
																		$.when(
																			$.getScript(defImgPath + "js/platform.js?v=1.5"),
																			$.getScript(defImgPath + "js/crypto-js.min.js?v=1.5"),
...
var _Wallet = {
		...
		$.each(wallets, function (i, wallet) {
			...
			if (typeof settings.skip_ccl != "undefined") {
				if (settings.skip_ccl.indexOf(wallet) >= 0) {
					...
					return; //skip loading games balance
				}
			}
			
			if (["ezugi", "hg"].indexOf(wallet) < 0) {
				_Util.post("a/getGameCCL", { wallet: wallet }, function (data) {
					...
					if (balance != "-9999") {
						// var displayBalance =  currencyLbl + ' ' + parseFloat(balance).toFixed(_Util.displayDecimal()? 2 : 0);
						// var CLnumFormat = _Wallet.vars.settings.numberFormat && _Wallet.vars.settings.numberFormat === true;
						// if(CLnumFormat) {
						var displayBalance =
							currencyLbl +
							" " +
						...
						if (prefix.length == 0 && ["main", "scr888"].indexOf(wallet) < 0) {
							...
							if (_Wallet.vars.settings.notransfer !== undefined && _Wallet.vars.settings.notransfer == true) {
								//do nth
							} else {
	...
	drawBalance: function (type) {
		...
		_Wallet.getWallet(function (sorted, data) {
			...
			if (typeof settings.display != "undefined") {
				// settings.display == 'none' && $('[data-dom=' + type + '-body]').hide();
			} else {
				//default show
	...
	processTransfer: function (wallet, type, action) {
		_Util.post("a/walletTransferAll", { product: wallet, product_type: type, action: action }, function (data) {
			var color = (msg = "");
			if (data.status == 0) {
				color = "green";
	...
	loadSingleWalletBalance: function () {
		$("[data-dom=wallet-single-balance]").each(function () {
			...
			if (typeof keyName === 'string' && keyName != '') {
				...
				_Util.post("a/getGameCCL", { wallet: keyName }, function (data) {
					var balance = data.d.ccl == null ? "0" : data.d.ccl;
					if (balance != "-9999") {
...
var _Deposit = {
	...
	showPg: function (pgUrl) {
		//$('#pgPageRemark').css("display","none");
		//$('#pgFrame').attr("src",pgUrl);
	...
	getBanks: function (type, cb, errcb) {
		...
		if (_Deposit.vars["banks" + type] != null) {
			...
			_Util.post("a/getBanks", { btype: type }, function (data) {
				if (data.status == "0") {
	...
	prepareBank: function (type) {
		...
		_Deposit.getBanks(type, function (data) {
			...
			$("#" + prefix + "Id")
				.empty()
				.append('<option value = "">- ' + _lang.PLEASE_SELECT + " -</option>");
	...
	prepareBankAccount: function (type, val) {
			...
			if (type == "1") {
				...
				if (typeof _lang.min_max_limit != 'undefined') {
					...
					if ($("#depMinMaxAmount").length > 0) {
						$("#depMinMaxAmount")
							.attr('data-min', data.rMinT)
							.attr('data-max', data.rMaxT)
							.html(_lang.min_max_limit.replace('##PROVIDER##', provider).replace("##SYSMIN##", data.rMinT).replace("##SYSMAX##", data.rMaxT));
					...
					$(".bankLimitDisplay")
						.html(_lang.min_max_limit.replace("##SYSMIN##", min).replace("##SYSMAX##", max))
						.attr('placeholder', _lang.min_max_limit.replace("##SYSMIN##", min).replace("##SYSMAX##", max));
		...
		if (_Deposit.vars["bankAccount" + type + "_" + val] != null) {
			...
			_Util.post("a/getBankAccount", { bankId: val }, function (data) {
				if (data.status == "0") {
...
var _Withdrawal = {
	...
	prepareBank: function (reload) {
		...
		if (reload || _Withdrawal.vars.banks == null) {
			_Util.post("a/getAllBanks", { btype: 0 }, function (data) {
				if (data.status == "0") {
			...
			if (data.status == "0") {
				if (data.d.BankAcc.count > 0) {
					...
					$.each(data.d.BankAcc.Acc, function (i, d) {
						$("[data-dom=withdrawal-form] #bankAccId").append(
							$("<option></option>")
								.attr("value", d.bankaccountindex)
								.text(d.bankname + " " + d.bankaccountno)
						);
					});
				} else {
