        ...
        switch (tabId) {
            case 0:
                if (allow_pg_cc == 0 && allow_pg_atm == 0) {
                    alertMSGCallback('網上支付暫未開放,請洽客服2', function () {
                        isreturnVip = true;
            ...
            if (result) {
                ...
                if (code == 0) {
                    if (o.data) {
                        ...
                        var ddl_pbank = $('.tab-content #supermarket-61 #ddl_pbank_mart');
                        if (ddl_pbank)
                            $(ddl_pbank).empty();
...
alertMSGDeposit = function (callback) {
    ...
    var content = "<div>";
    content += "<p style='font-size:18px;'><strong>儲值優惠須於尚未遊戲前找客服領取，否則視同放棄</strong><p>";
    content += "</div>";
...
alertMSGDepositMartPromo = function (callback) {
    ...
    var content = "<div>";
    content += "<p style='font-size:18px;'><strong>請拍照繳費收據，上傳客服才能上分唷<br>（首次超商儲值，請備妥雙證件喔！）</strong><p>";
    content += "<br /><p style='font-size:18px;'><strong>儲值優惠須於尚未遊戲前找客服領取，否則視同放棄</strong><p>";
    content += "</div>";
