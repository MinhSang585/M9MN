...
function onekeyMonery() {
    ...
    $('.btn_onekeytranfer').prop('disabled',true);
    ...
    if (jsonData == null || jsonData == "" || typeof jsonData == "undefined" || jsonData == '""') {
        ...
        $('.btn_onekeytranfer').prop('disabled',false);
...
function setCustomerService() {
    ...
    $(document).on("click", "#live_in", function (e) {
        layer.open({
            ...
            btnAlign: 'c',
            btn: ['快速登入', '联系客服'],
            content: '<div class="fast-tip-cont">亲爱的iWin国际玩家：<br><p class="suggest-cont">建议您先登入您的游戏账号，可以加快在线客服的处理速度。</p></div>',

            yes: function () {
                openLoginModule();
            },
            btn2: function () {
                getCsOnDutyAi('pc', '<%=infoValue%>');
            }
        })
    });
    ...
    $(document).on("click", "#phone_in", function (e) {
        ...
        document.getElementById("iframePhone").src = "/mobile.jsp";
...
function setCookie(cname, cvalue, ctime) {
    ...
    if (typeof cvalue == "undefined" || cvalue == "请登入后再操作") {
        return false;
    }
    ...
    var d = new Date();
    d.setTime(d.getTime() + (1000 * 60 * ctime)); // 1000毫秒 * 60秒 * 30 ＝ 30min
