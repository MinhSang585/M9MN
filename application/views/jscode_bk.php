<script type="text/javascript">
	...
	if($this->session->userdata('is_logged_in') == true):
		if(!$this->session->userdata('read_notice')) {
	?>
	$("#announcementPopup1Button").click();
	<?php
		}
	endif;
	?>
	...
	$("#announcementBtn1").click(function() {
		$.ajax({url: "<?php echo site_url('ajax/set_read_announcement');?>",
			type: 'get',
			async: 'true',
			cache: false,
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#mobile").keyup(function(){
			...
			if(mobile != ''){
				$.ajax({url: "<?php echo base_url('custom/check_register_mobile/');?>" + mobile,
					type: 'get',
					async: 'true',
					success: function (data) {
						var json = JSON.parse(JSON.stringify(data));
						...
						if(json.status == '<?php echo EXIT_ERROR;?>') {
							layer.alert(
								message, {
								icon: 2, title: '<?php echo $this->lang->line('label_info');?>',
		...
		$("#bank_account_no").keyup(function(){
			...
			if(bank_account_no != ''){
				$.ajax({url: "<?php echo base_url('custom/check_player_bank/');?>" + bank_account_no,
					type: 'get',
					async: 'true',
					success: function (data) {
						var json = JSON.parse(JSON.stringify(data));
			...
			if(is_allowed == true) {
				...
				$.ajax({url: player_bank_form.attr('action'),
					...
					success: function (data) {
						...
						if(json.status == '<?php echo EXIT_SUCCESS;?>') {
							layer.alert(message, {icon: 1, title: '<?php echo $this->lang->line('label_info');?>', btn: '<?php echo $this->lang->line('label_close');?>', closeBtn: 0}, function (){ location.reload(); layer.closeAll(); is_allowed = true; });
						}
						else
						{
							layer.alert(message, {icon: 2, title: '<?php echo $this->lang->line('label_info');?>', btn: '<?php echo $this->lang->line('label_close');?>', closeBtn: 0}, function (){ layer.closeAll(); is_allowed = true; });
		...
		?>
			sub_game('<?php echo $this->uri->segment(3);?>', '<?php echo GAME_SLOTS;?>');
		...
		?>
			sub_game('<?php echo $this->uri->segment(3);?>', '<?php echo GAME_FISHING;?>');
...
function coming_soon() {
	layer.alert('<?php echo $this->lang->line('label_coming_soon');?>', {icon: 0, title: '<?php echo $this->lang->line('label_info');?>', btn: '<?php echo $this->lang->line('label_close');?>'}, function (){ layer.closeAll(); });
}

function nav_redirect(param) {
	$.ajax({url: "<?php echo site_url('ajax/check_login');?>",
		type: 'get',
		dataType: 'json',
		cache: false,
		async: 'true',
		...
		success: function (data) {
			if(data.status == 'true') {
				if(param == "deposit") {
					location.href = "<?php echo site_url('account/deposit');?>";
				}
				else if(param == "withdraw") {
					location.href = "<?php echo site_url('account/withdrawal');?>";
				}
				else if(param == "wallet") {
					location.href = "<?php echo site_url('account/wallet');?>";
				}
				else {
					location.href = "<?php echo site_url('account/transaction_history');?>";
				}
			}
			else
			{
				layer.alert('<?php echo $this->lang->line('error_please_login_to_continue');?>', {icon: 0, title: '<?php echo $this->lang->line('label_info');?>', btn: '<?php echo $this->lang->line('label_close');?>'}, function (){ layer.closeAll(); });
			}
		},
		error: function (request,error) {
			console.log(request);
...
<?php if($this->uri->segment(1) == 'register' || $this->uri->segment(1) == 'account'):?>
	$(document).ready(function() {
		$('#dob').datetimepicker({
			format: 'Y-m-d',
			timepicker: false
		...
		var dob_form = $('#dob-form');

		dob_form.submit(function(event) {
			$.ajax({url: dob_form.attr('action'),
				data: dob_form.serialize(),
				type: 'post',
				dataType: 'json',
				cache: false,
				async: 'true',
...
<?php if($this->uri->segment(1) == 'register' || $this->uri->segment(1) == 'account'):?>
	$(document).ready(function() {
		...
		email_form.submit(function(event) {
			$.ajax({url: email_form.attr('action'),
