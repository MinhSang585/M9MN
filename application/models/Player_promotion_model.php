...
class Player_promotion_model extends CI_Model {
	...
	public function updateStatus($player_id)
	{	
		$this->db->set('status', STATUS_CANCEL);
		$this->db->where('player_id', $player_id);
		$this->db->where_in('status', [STATUS_PENDING]);
		$this->db->where_in('genre_code', ['DE', 'DPR']);
		$this->db->update($this->table_player_promotion);
	}

	public function checkTypePromotion($playerId)
	{
		$this->db->select('genre_code');
		$this->db->where('player_id', $playerId);

		$genreCodes = array(
			PROMOTION_TYPE_UDB,
			PROMOTION_TYPE_WRLV,
			PROMOTION_TYPE_MEGA_SL,
			PROMOTION_TYPE_PUS8
		);
		$this->db->group_start();
			$this->db->or_where_in('genre_code', $genreCodes);
			$this->db->or_where_in('game_provider_type_code', $genreCodes);
		$this->db->group_end();
		$query = $this->db->get($this->table_player_promotion);
		
		$result = $query->row();
		return $result ? $result->genre_code : null;
	}

	public function getTypePromotion($playerId, $typePromotion){
		$result = array();
		// $this->db->where('is_reward', STATUS_ACTIVE);
		// $this->db->where('active', STATUS_ACTIVE);
		$this->db->where('player_id', $playerId);
		$this->db->where('genre_code', $typePromotion);
		$this->db->like('apply_type', ','.PROMOTION_USER_TYPE_ADMIN.',');
	...
	public function get_player_promo($player_id, $genre_code)
	{
		$result = array();
		$this->db->select('player_id, promotion_id, created_date');
		$this->db->where('player_id', $player_id);
		$this->db->where('genre_code', $genre_code);
