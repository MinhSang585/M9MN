...
class MY_Controller extends CI_Controller
{
    ...
    {
        parent::__construct();
		$this->load->model(array('api_model', 'game_model', 'miscellaneous_model', 'player_model', 'report_model'));
    }
	public function curl_get($url = NULL, $token = NULL) 
	{
	    if(is_array($token)){
			...
			$headers = array_merge($headers,$token);
	    }else{
	        $headers = array(
				'charset=UTF-8',
				'Content-Type: application/x-www-form-urlencoded',
	...
	{
		$data_string = ((is_array($arr)) ? json_encode($arr) : $arr); 
		$ch = curl_init($url);  	
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");                                                                      
		curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);  
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
	...
	public function curl_post_for_allbet($url = NULL, $arr = NULL, $header = NULL) 
	{
		$data_string = json_encode($arr);
		$ch = curl_init($url);  	
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");                                                                      
	...
	public function curl_soap_version_one($url = NULL, $arr = NULL, $header = NULL){
	    $headers = array();
	    if(isset($header['action'])){
	        array_push($headers,'SOAPAction: '.$header['action']);   
	    }
	    if(isset($header['authorization'])){
	        array_push($headers,'Authorization: '.$header['authorization']);   
	    }
	    if(isset($header['content_type'])){
	        array_push($headers,'Content-Type: '.$header['content_type']);   
	    }
	    $data_string = ((is_array($arr)) ? json_encode($arr,true) : $arr);
	    $ch = curl_init($url);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	...
	public function curl_json_km($url = NULL, $arr = NULL, $token = NULL) 
	{
	    $data_string = ((is_array($arr)) ? json_encode($arr) : $arr); 
	...
	public function curl_get_json_km($url = NULL, $token = NULL) 
	{
		$ch = curl_init($url);
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
		curl_setopt($ch, CURLOPT_TIMEOUT, 40);
